...
class OutgoingProductRepository
{
    ...
    {
        return $this->model
            ->with('product_location', 'process_plan')
            ->where('amount', 'LIKE', '%' . $term . '%')
            ->orWhereHas('product_location', function ($query) use ($term) {
    ...
    public function all()
    {
        return $this->model->with('product_location', 'process_plan')->get();
    }

    public function paginate(int $num)
    {
        return $this->model->with('product_location', 'process_plan')->paginate($num);
    }
